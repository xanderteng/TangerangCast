...
def _rain_by_hour(df: pd.DataFrame, container) -> None:
    if "Rain" not in df.columns or "_time" not in df.columns:
        return
    tmp = df.copy()
    tmp["Hour"] = tmp["_time"].dt.hour
    grouped = tmp.groupby("Hour")["Rain"].mean().reset_index()
    grouped["Rain %"] = (grouped["Rain"] * 100).round(1)
    container.bar_chart(grouped.set_index("Hour")["Rain %"], color=_CLR["rain"])
...
def _section_current() -> None:
    ...
    left.markdown("** Rain Distribution**")
    _rain_bar(df, left)
    right.markdown("** Grid Coverage Map**")
    _spatial_map(df, right)

    st.markdown("**📊 Feature Histograms (Current Snapshot)**")
    hcols = st.columns(3)
    for i, feat in enumerate(["Temperature", "Humidity", "Wind_Speed"]):
        if feat in df.columns:
            hcols[i].markdown(f"*{feat} ({_COL_UNITS[feat]})*")
            hcols[i].bar_chart(
                df[feat].dropna().value_counts(bins=15, sort=False).sort_index(),
                color=_COL_COLORS[feat],
            )
...
def _section_historic() -> None:
    ...
    st.markdown("** Rain Probability by Hour of Day**")
    _rain_by_hour(df, st)
...
def _section_future() -> None:
    ...
    st.markdown("** Forecast Rain Outlook**")
    _rain_bar(df, st)
...
def _section_bulk_export() -> None:
    ...
    st.caption("Download all available raw datasets in one click.")
    tiers = {
        "Current":  ("current",  "tangerangcast_current"),
        "Historic": ("historic", "tangerangcast_historic"),
        "Forecast": ("future",   "tangerangcast_forecast"),
    }
    ecols = st.columns(len(tiers))
    dummy_fns = [_make_dummy_current, _make_dummy_historic, _make_dummy_future]
    for (label, (tier, base)), dummy_fn, col in zip(tiers.items(), dummy_fns, ecols):
        df = _load_tier(tier) or dummy_fn()
        export_df = df.drop(columns=["_time"], errors="ignore")
        fname = f"{base}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        col.download_button(
            label=f"⬇️ {label}",
            data=_to_csv(export_df),
            file_name=fname,
...
with st.sidebar:
    ...
    st.info(
        "⚠️ **Note**\n\n"
        "Sections without real data display dummy visualisations "
...
with st.expander("📁 Data Availability", expanded=True):
    bcols = st.columns(3)
    tier_labels = [("current", " Current"), ("historic", " Historic"), ("future", " Forecast")]
    for col, (tier, label) in zip(bcols, tier_labels):
        paths = _list_csvs(_DIRS.get(tier, ""))
        if paths:
